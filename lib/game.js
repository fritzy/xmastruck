...
function XmasTruck(canvas) {
    ...
    for (var x = 0; x < 6000; x++) {
        this.lines.push({x: x*40, y: this.terrain[x] * 4});
...
(function () {
    ...
    this.tick = function () {
        ...
        var pos = this.objects.truck.wheel2.getPos();
        var start = Math.floor((pos.x  - 650)/ 40);
        
        this.renderer.ctx.beginPath();
        this.renderer.ctx.moveTo((this.lines[start].x - pos.x + 600) * .3, (this.lines[start].y - pos.y + 1300) * .3);
        for(var x = start; x < start + 80; x++) {
            this.renderer.ctx.lineTo((this.lines[x].x - pos.x + 600) * .3, (this.lines[x].y - pos.y + 1300) * .3); 
        }
